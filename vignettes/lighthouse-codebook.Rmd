...
## Creating a codebook
In the simplest case, to create a codebook and write it to disk:.
```r
gain_q4 |> cb_create() |> cb_write("q4.xlsx")
```

Creating a codebook using the lighthouse.codebook package involves two steps.

1) Create a a "codebook" object from a data frame (and, optionally, metadata) using 
`cb_create()` or a specialized variant (such as `cb_create_spss()` or `cb_create_redcap()`).
...
A codebook written to disk will include an Overview tab as well as Summary tabs 
for numeric, categorical, and text variables. The Overview tab includes one row 
for each variable in the dataset, with information on variable type, label, values, 
and missingness. By default, each variable is hyperlinked to its location on the 
relevant summary tab.
