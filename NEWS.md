...
* In grouped numeric summaries in workbooks generated by `cb_write()`, some or 
  all grouping variables can now be shown in rows rather than columns by setting
  the `group_rows_numeric` argument.
